...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    //call resize function initially to figure out the initial width. Other resize events handled by event listener using callback function. 
    handleResize();

    //in React when using eventlistener above you need to remove it.
    return () => window.removeEventListener('resize', handleResize);
  }, []);

  //conditional statement checking if screen size is small and removing sidebar from screen.
  useEffect(() => {
    if(screenSize <= 900) {
      setActiveMenu(false);
    } else {
